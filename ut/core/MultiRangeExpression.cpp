...
class MultiRangeExpression_test : public ::testing::Test {
   public:
    ...
    MultiRangeExpression_test() { multirange_.setLiteral(LITERAL); }
};

TEST_F(MultiRangeExpression_test, getLiteral) { EXPECT_EQ(multirange_.getLiteral(), std::string(LITERAL)); }
