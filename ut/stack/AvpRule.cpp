...
TEST_F(AvpRule_test, getAvpName) { EXPECT_EQ(avp_rule_->getAvpName(), "Auth-Application-Id"); }

TEST_F(AvpRule_test, getPresence) {
    EXPECT_EQ(avp_rule_->getPresence(), ert::diametercodec::stack::AvpRule::Presence::Optional);
}

TEST_F(AvpRule_test, getQual) { EXPECT_EQ(avp_rule_->getQual(), "1*"); }

TEST_F(AvpRule_test, isAny) { EXPECT_EQ(avp_rule_->isAny(), false); }

TEST_F(AvpRule_test, isFixed) { EXPECT_EQ(avp_rule_->isFixed(), false); }

TEST_F(AvpRule_test, isMandatory) { EXPECT_EQ(avp_rule_->isMandatory(), false); }

TEST_F(AvpRule_test, isOptional) { EXPECT_EQ(avp_rule_->isOptional(), true); }

TEST_F(AvpRule_test, getQualMin) { EXPECT_EQ(avp_rule_->getQualMin(), 1); }

TEST_F(AvpRule_test, getQualMax) { EXPECT_EQ(avp_rule_->getQualMax(), -1); }
