...
cd ${git_root_dir}
rm -rf coverage
docker build --rm ${bargs} -f Dockerfile.coverage -t ${registry}/diametercodec:latest-cov . || exit 1
docker run -it --rm -v ${PWD}/coverage:/code/coverage ${registry}/diametercodec:latest-cov || exit 1
